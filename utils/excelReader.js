...
/**
 * Automatically increments dates in 'AttendanceRecords' sheet for repeat runs.
 * Jumps forward by the total number of rows to prevent date collisions.
 */
export function incrementExcelDates(filePath) {
    const workbook = xlsx.readFile(filePath);
    const sheetName = 'AttendanceRecords';
    ...
    // Calculate the jump based on the total number of rows in the file
    const daysToJump = rawData.length; 

    const updatedData = rawData.map(row => {
        if (!row.serviceDate) return row;

        if (typeof row.serviceDate === 'number') {
            return { ...row, serviceDate: row.serviceDate + daysToJump };
        } else {
            const dateObj = new Date(row.serviceDate);
            if (!isNaN(dateObj.getTime())) {
                dateObj.setDate(dateObj.getDate() + daysToJump);
                const month = String(dateObj.getMonth() + 1).padStart(2, '0');
                const day = String(dateObj.getDate()).padStart(2, '0');
                const year = dateObj.getFullYear();
                return { ...row, serviceDate: `${month}/${day}/${year}` };
    ...
    workbook.Sheets[sheetName] = newSheet;
    xlsx.writeFile(workbook, filePath);
    console.log(`[SYSTEM] Excel dates successfully incremented by +${daysToJump} days in AttendanceRecords.`);
}